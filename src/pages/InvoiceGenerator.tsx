    ...
    <div className="p-8 animate-fade-in max-w-4xl">
      ...
      <div className="space-y-6">
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id}>
                    ...
                    <td className="font-medium">${(item.quantity * item.unitPrice).toFixed(2)}</td>
                    <td>
                      <button
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button variant="outline" onClick={() => handleCreate("draft")}>Save as Draft</Button>
          <Button onClick={() => handleCreate("pending")}>Generate Invoice</Button>
        </div>
      </div>
    </div>
  );
